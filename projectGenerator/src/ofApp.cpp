...
namespace PG{
	...
	bool checkConfigExists(){
		ofFile config(ofFilePath::join(ofFilePath::getCurrentExeDir(), ".ofprojectgenerator/config"));
	...
	bool askOFRoot(){
		...
		ofDirectory config(ofFilePath::join(ofFilePath::getCurrentExeDir(), ".ofprojectgenerator"));
		config.create(true);
		ofFile configFile(ofFilePath::join(ofFilePath::getCurrentExeDir(), ".ofprojectgenerator/config"), ofFile::WriteOnly);
		configFile << res.filePath;
		return true;
	}

	string getOFRootFromConfig(){
		if (!PG::checkConfigExists()) return "";
		ofFile configFile(ofFilePath::join(ofFilePath::getCurrentExeDir(), ".ofprojectgenerator/config"), ofFile::ReadOnly);
...
void ofApp::setup(){
	...
	while(!PG::checkConfigExists()){
		PG::askOFRoot();
	}

	setOFRoot(PG::getOFRootFromConfig());
...
void ofApp::changeOFRootPressed(){
	PG::askOFRoot();
	cout << PG::getOFRootFromConfig()<<endl;
	setOFRoot(PG::getOFRootFromConfig());
	setupDrawableOFPath();
}
